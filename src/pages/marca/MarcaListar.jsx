// src/pages/productos/marca/MarcaListar.jsx
...
export default function MarcaListar() {
  const navigate = useNavigate();

  const [data, setData] = useState([]);      // [{ id_marca, nombre_marca }]
  ...
  const [toDelete, setToDelete] = useState(null); // { id_marca, nombre_marca }
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((m) =>
      [m.nombre_marca, String(m.id_marca)].some((v) =>
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await guardedFetch(
        `${API_URL}/api/marca/delete/${toDelete.id_marca}`,
        { method: "DELETE" }
      );
      if (res.status === 204) {
        setData((d) => d.filter((x) => x.id_marca !== toDelete.id_marca));
      } else {
    ...
    <section className="w-full">
      ...
      <div className="mx-auto max-w-5xl space-y-4 sm:space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <h1 className="text-2xl md:text-3xl font-semibold">Lista de Marcas</h1>

          <Link
            to="/productos/marca/registrar-marca"
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                ...
                {pageItems.map((m, i) => (
                  <tr key={m.id_marca} className="border-t last:border-b">
                    <td className="px-4 py-3">{(page - 1) * pageSize + i + 1}</td>
                    <td className="px-4 py-3">{m.nombre_marca}</td>
                    <td className="px-4 py-3">
                      <div className="flex justify-center gap-2">
                        <button
                          onClick={() => navigate(`/productos/marca/editar-marca/${m.id_marca}`)}
      ...
      <ConfirmModal
        ...
        message={
          toDelete
            ? `¿Seguro que deseas eliminar la marca "${toDelete.nombre_marca}"? Esta acción no se puede deshacer.`
